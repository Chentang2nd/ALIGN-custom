...
def gen_param(subckt, primitives, pdk_dir):
    ...
    if generator_name == 'CAP':
        ...
        for key in mvalues:
            ...
            # For FinFET technologies with Fin_pitch, divide by pitch; for planar technologies, use width directly
            if "Fin_pitch" in design_config:
                width = int(float(mvalues[key]["W"])*1E+9/design_config["Fin_pitch"])
            else:
                width = int(float(mvalues[key]["W"])*1E+9)
        ...
        if 'NF' in mvalues[device_name].keys():
            for key in mvalues:
                ...
                # Only enforce even NF for FinFET technologies with Fin_pitch
                if "Fin_pitch" in design_config:
