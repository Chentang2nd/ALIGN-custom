...
class MOSGenerator(DefaultCanvas):
    ...
    def __init__(self, pdk, height, fin, gate, gateDummy, shared_diff, stack, bodyswitch, **kwargs):
        ...
        # Detect if this is a FinFET or planar technology
        try:
            self.is_finfet = 'Fin' in pdk and 'Pitch' in pdk['Fin']
        except (KeyError, TypeError):
            self.is_finfet = False
        ...
        # For planar technologies, use a minimal reasonable height
        # For FinFET, use fin-based calculation
        if not self.is_finfet:
            # For planar CMOS, height should be minimal (around 1-4 M2 tracks)
            # Standard cell height is typically 4 tracks high
            height = 4
        else:
        ...
        for const in self.primitive_constraints:
            if const.constraint == 'Generator':
                if const.parameters is not None:
                    ...
                    # Only allow height override for FinFET, not planar
                    if const.parameters.get('height') and self.is_finfet:
        ...
        if self.is_finfet:
            ...
            # For planar technology, unitCellHeight should be based on M2 pitch multiples
            # height is number of M2 tracks, each M2['Pitch'] nanometers
            self.m2PerUnitCell = height
            self.unitCellHeight = self.m2PerUnitCell * self.pdk['M2']['Pitch']
        ######### Derived Parameters ############
        ...
        if self.is_finfet:
            ...
            # For planar technology
            self.finDummy = 0
            self.lFin = 0
            # For planar, use Active layer offset and width
            activeOffset = self.pdk['Active']['Offset'] if 'Offset' in self.pdk['Active'] else 0
            activeWidth = exact_width if exact_width else self.pdk['Active']['Width']
        ...
        # For planar technology, use smaller extension; for FinFET use larger
        if self.is_finfet:
            poly_stoppoint = self.pdk['Active']['activePolyExTracks']*self.pdk['M2']['Pitch']
            m1_stoppoint = self.pdk['V1']['VencA_L'] +self.pdk['M2']['Width']//2
        else:
            # For planar, minimize extension to Active
            poly_stoppoint = self.pdk['Active']['PolyExtension']  # Use PolyExtension directly
            m1_stoppoint = self.pdk['Active']['PolyExtension']  # M1 also respects PolyExtension
        ...
        self.pl = self.addGen( Wire( 'pl', 'Poly', 'v',
                                     ...
                                     spg=EnclosureGrid( pitch=self.unitCellHeight, offset=self.pdk['M2']['Offset'], stoppoint=poly_stoppoint, check=True)))
        ...
        self.m1_updated = self.addGen( Wire( 'm1_updated ', 'M1', 'v',
                                     ...
                                     spg=EnclosureGrid( pitch=self.pdk['M2']['Pitch'], stoppoint=m1_stoppoint, check=False)))
        ...
        # Only generate Fin layer for FinFET technologies
        if self.is_finfet:
        ...
        # For FinFET use fin.clg, for planar use active.clg
        select_h_grid = self.fin.clg if self.is_finfet else self.active.clg
        ...
        if self.is_finfet:
            ...
            # For planar, offset should be half of unit cell height
            offset_active_body = self.unitCellHeight // 2
    ...
    def addNMOSArray( self, x_cells, y_cells, pattern, vt_type, connections, **parameters):
        ...
        # Use m2PerUnitCell for planar, finsPerUnitCell for FinFET
        region_height = y_cells * self.m2PerUnitCell if not self.is_finfet else y_cells * self.finsPerUnitCell
        region_height2 = self.bodyswitch * self.m2PerUnitCell if not self.is_finfet else self.bodyswitch * self.lFin

        self.addRegion( self.nselect, None, -M3_tracks_start, 0, M3_tracks_end, region_height)
        if self.bodyswitch==1:self.addRegion( self.pselect, None, -M3_tracks_start, region_height, M3_tracks_end, region_height+region_height2)
    ...
    def addPMOSArray( self, x_cells, y_cells, pattern, vt_type, connections, **parameters):
        ...
        self.addRegion( self.pselect, None, -M3_tracks_start, 0, M3_tracks_end, region_height)
        if self.bodyswitch==1:self.addRegion( self.nselect, None, -M3_tracks_start, region_height, M3_tracks_end, region_height+region_height2)
        self.addRegion( self.nwell, None, -M3_tracks_start, 0, M3_tracks_end, region_height+region_height2)
